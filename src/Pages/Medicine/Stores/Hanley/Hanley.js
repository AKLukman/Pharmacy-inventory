...
const Hanley = () => {
  ...
  const handleCheckId = (data) => {
    ...
    if (date && users?.store === "hanley" && age > 17) {
      console.log("success");
      navigate("/allmedicine");
    } else if (date !== inputDate) {
      console.log("failed");

      toast("User not registered at this store!.", {
        style: {
      ...
      setNotMached("User not registered at this store!");
      setUnderAge("");
      setStore("");
      reset();
    } else if (date && users?.store === "hanley" && age < 18) {
      ...
      setStore("");
      reset();
    } else if (date && users?.store !== "hanley") {
  ...
  return (
    <div className="h-[500px] flex justify-center items-center">
      <div className="w-96 p-7">
        <h2 className="text-xl text-center  font-bold">Hanley Store</h2>
