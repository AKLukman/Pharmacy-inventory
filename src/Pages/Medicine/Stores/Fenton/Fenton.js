...
const Fenton = () => {
  ...
  const handleCheckId = (data) => {
    ...
    if (date && users?.store === "fenton" && age > 17) {
      console.log("success");
      ...
      setStore("");
      reset();
    } else if (date && users?.store === "fenton" && age < 18) {
      toast("User registered but under aged!.", {
        style: {
          backgroundColor: "white",
          color: "red",
          fontSize: "bold",
      ...
      setUnderAge("User Registered but under aged!");
      setNotMached("");
      setStore("");
      reset();
    } else if (date && users?.store !== "fenton") {
  ...
  return (
    <div className="h-[500px] flex justify-center items-center">
      <div>
        ...
        <Link to={"/fenton-store-not-id-check"}>
      ...
      <div className="w-96 p-7">
        <h2 className="text-xl text-center  font-bold">Fenton Store</h2>
