...
const HanleyMedicineDetails = () => {
  ...
  const {
    ...
    queryKey: [`hanley-store`],
    queryFn: async () => {
      const res = await fetch(
        `http://localhost:5000/api/v1/pharmacy/hanley-store/${medicineId._id}`
      );
      ...
      <div ref={contentRef}>
        <div class="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
          <div class="border-t-2 border-b-2 py-4 mb-8">
            ...
            <p class="text-sm text-center mt-2">Hanley Store</p>
          </div>
            ...
            <table class="w-full mt-4">
              ...
              <tbody>
                <tr>
                  ...
                  <td class="text-sm">{medicine.condition}</td>
  ...
  const storeName = "hanley";
  useEffect(() => {
    fetch(
      `http://localhost:5000/api/v1/pharmacy/userHanley/${dateOfBirth}/${storeName}`
    )
      .then((res) => res.json())
  ...
  const handleCheckId = (data) => {
    ...
    if (date && user?.store === "hanley" && age > 17) {
      console.log("success");
      ...
      setUserRegisterd("");
      reset();
    } else if (date && user?.store === "hanley" && age < 18) {
      ...
      setUserRegisterd("");
      reset();
    } else if (date && user?.store !== "hanley") {
  ...
  let details = (
    <div>
      ...
      <div className="mb-4">
        ...
        <p>
          Availability in stoke store:
          <span className="font-bold"> {medicine.availabilityHanley}</span>
        </p>
      </div>
      <div class="w-1/3">
  ...
  return (
    <div className="m-[50px]">
      ...
      {medicine.idCheck === "Y" && mama === false ? (
        <div>
          <h2 className="text-4xl m-10 text-center font-bold uppercase text-red-400">
            Hanley Store ID CHECK
          </h2>
...
export default HanleyMedicineDetails;
