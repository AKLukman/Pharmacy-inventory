...
const TunstallMedicineDetails = () => {
  ...
  const {
    ...
    queryKey: [`tunstall-store`],
    queryFn: async () => {
      const res = await fetch(
        `http://localhost:5000/api/v1/pharmacy/tunstall-store/${medicineId._id}`
      );
      ...
      <div ref={contentRef}>
        <div class="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
          <div class="border-t-2 border-b-2 py-4 mb-8">
            ...
            <p class="text-sm text-center mt-2">Tunstall Store</p>
          </div>
  ...
  const handleCheckId = (data) => {
    ...
    if (date && user?.store === "tunstall" && age > 17) {
      console.log("success");
      ...
      setUserRegisterd("");
      reset();
    } else if (date && user?.store === "tunstall" && age < 18) {
      ...
      setUserRegisterd("");
      reset();
    } else if (date && user?.store !== "tunstall") {
  ...
  let details = (
    <div>
      ...
      <div className="mb-4">
        ...
        <p>
          Availability in Tunstall store:
          <span className="font-bold"> {medicine.availabilityTunstall}</span>
        </p>
      </div>
      <div class="w-1/3">
...
export default TunstallMedicineDetails;
